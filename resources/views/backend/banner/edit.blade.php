...
<div class="card">
    <h5 class="card-header">Banner o'zgartirsh</h5>
    <div class="card-body">
      <form method="post" action="{{route('banner.update',$banner->id)}}">
        ...
        <input id="inputTitle" type="text" name="title" placeholder="Sarlavhani kiriting"  value="{{$banner->title}}" class="form-control">
        @error('title')
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            <option value="active" {{(($banner->status=='active') ? 'selected' : '')}}>Faol</option>
            <option value="inactive" {{(($banner->status=='inactive') ? 'selected' : '')}}>Faol emas</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
