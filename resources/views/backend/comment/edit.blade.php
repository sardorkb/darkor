...
<div class="card">
  <h5 class="card-header">Commentni tahrirlash</h5>
  <div class="card-body">
    <form action="{{route('comment.update',$comment->id)}}" method="POST">
      ...
      <div class="form-group">
        ...
        <select name="status" id="" class="form-control">
          ...
          <option value="active" {{(($comment->status=='active')? 'selected' : '')}}>Fao;</option>
          <option value="inactive" {{(($comment->status=='inactive')? 'selected' : '')}}>Faol emas</option>
        </select>
      </div>
